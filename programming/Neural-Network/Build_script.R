...
nnet <- function(X, Y, step_size = 0.5, reg = 0.001, h = 10, niteration){  # get dim of input  
    ...
    {    # hidden layer, ReLU activation    
      ...
      scores <- hidden_layer%*%W2 + matrix(rep(b2,N), nrow = N, byrow = T)     # compute and normalize class probabilities    
...
ggplot()+  geom_tile(aes(x = grid[,1],
                     ...
                     alpha = 0.3, show.legend = F)+ geom_point(data = data, 
                                                                 ...
                                                                 size = 2) + theme_bw(base_size = 15) +  ggtitle('Neural Network Decision Boundary') +  coord_fixed(ratio = 0.8) +   theme(axis.ticks=element_blank(),
                                                                                                                                                                                           panel.grid.major = element_blank(),
                                                                                                                                                                                           panel.grid.minor = element_blank(),
                                                                                                                                                                                           axis.text=element_blank(),
                                                                                                                                                                                           axis.title=element_blank(),
